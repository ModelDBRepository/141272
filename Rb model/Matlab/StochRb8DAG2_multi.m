    ...
    for nn=1:10
        ...
        for t = dt:dt:Tstop
            ...
            for ii = 1:nsim  %because this algorithm needs a matrix operation (line 90) it has to be done iterating over independent simulations
                ...
                Dmtx=[3*ami.*sa(1,ii)+(2*ami+bmi+ahi).*sa(2,ii)+2*bmi.*sa(3,ii)+bhi.*sa(6,ii),-2*(ami.*sa(2,ii)+bmi.*sa(3,ii)),0,0,-(ahi.*sa(2,ii)+bhi.*sa(6,ii)),0,0;
                    -2*(ami.*sa(2,ii)+bmi.*sa(3,ii)),2*ami.*sa(2,ii)+(ami+2*bmi+ahi).*sa(3,ii)+3*bmi.*sa(4,ii)+bhi.*sa(7,ii),-(ami.*sa(3,ii)+3*bmi.*sa(4,ii)),0,0,-(ahi.*sa(3,ii)+bhi.*sa(7,ii)),0;
                    0,-(ami.*sa(3,ii)+3*bmi.*sa(4,ii)),ami.*sa(3,ii)+(3*bmi+ahi).*sa(4,ii)+bhi.*sa(8,ii),0,0,0,-(ahi*sa(4,ii)+bhi.*sa(8,ii));
                    0,0,0,ahi.*sa(1,ii)+(3*ami+bhi).*sa(5,ii)+bmi.*sa(6,ii),-(3*ami.*sa(5,ii)+bmi.*sa(6,ii)),0,0;
                    -(ahi.*sa(2,ii)+bhi.*sa(6,ii)),0,0,-(3*ami.*sa(5,ii)+bmi.*sa(6,ii)),ahi.*sa(2,ii)+3*ami.*sa(5,ii)+(2*ami+bmi+bhi).*sa(6,ii)+2*bmi.*sa(7,ii),-2*(ami.*sa(6,ii)+bmi.*sa(7,ii)),0;
                    0,-(ahi.*sa(3,ii)+bhi.*sa(7,ii)),0,0,-2*(ami.*sa(6,ii)+bmi.*sa(7,ii)),ahi.*sa(3,ii)+2*ami.*sa(6,ii)+(ami+2*bmi+bhi).*sa(7,ii)+3*bmi.*sa(8,ii),-(ami.*sa(7,ii)+3*bmi.*sa(8,ii));
                    0,0,-(ahi*sa(4,ii)+bhi.*sa(8,ii)),0,0,-(ami.*sa(7,ii)+3*bmi.*sa(8,ii)),ahi.*sa(4,ii)+ami.*sa(7,ii)+(3*bmi+bhi).*sa(8,ii)]/NNa;
                          
                Rvec(:,ii)=chol(dt*Dmtx)*randn(7,1);
            end
...
csvwrite(sprintf('EffN-Rb8DAG2-N%g-dt%g-%gs.csv',NNa,dt,realt),[currents' Eff meanFT varFT NaNs])
